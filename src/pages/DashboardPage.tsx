...
export function DashboardPage() {
  ...
  const paidRevenue = (month: string) => invoices.filter((invoice) => invoice.issueDate.startsWith(month) && invoice.status === 'paid').reduce((sum, invoice) => sum + invoice.total, 0);
  ...
  const months = Array.from({ length: 6 }, (_, index) => {
    const date = new Date(2026, 8 - (5 - index), 1);
    const key = monthKey(date);
    return { month: new Intl.DateTimeFormat('de-CH', { month: 'short' }).format(date).replace('.', ''), umsatz: paidRevenue(key), offen: invoices.filter((invoice) => invoice.issueDate.startsWith(key) && ['sent', 'partial', 'overdue'].includes(invoice.status)).reduce((sum, invoice) => sum + invoice.total - invoice.paidAmount, 0) };
  });
  ...
  const kpis = [
    ...
    { label: 'Offene Rechnungen', value: formatCurrency(outstanding.reduce((sum, invoice) => sum + invoice.total - invoice.paidAmount, 0), currency), detail: `${outstanding.length} Rechnungen ausstehend`, icon: ReceiptText },
    { label: 'Überfällige Rechnungen', value: String(overdue.length), detail: formatCurrency(overdue.reduce((sum, invoice) => sum + invoice.total, 0), currency), icon: ReceiptText, alert: true },
    { label: 'Offene Kostenvoranschläge', value: String(openQuotes.length), detail: formatCurrency(openQuotes.reduce((sum, quote) => sum + quote.total, 0), currency), icon: FileCheck2 },
  ...
  const actions = [
    ...
    { label: 'Neuer Kostenvoranschlag', icon: FileCheck2, path: '/kostenvoranschlaege?neu=1' },
    { label: 'Neuer Auftrag', icon: Wrench, path: '/auftraege?neu=1' },
    { label: 'Neue Rechnung', icon: ReceiptText, path: '/rechnungen?neu=1' },
    { label: 'Zeit erfassen', icon: Clock3, path: '/zeiterfassung?neu=1' },
  ];

  return (
    <div className="page dashboard-page">
      <PageHeader eyebrow="Sonntag, 13. September 2026" title="Guten Morgen, Sarah" description="Hier sehen Sie, was heute in Ihrem Betrieb wichtig ist." actions={<Button icon={Plus} onClick={() => navigate('/auftraege?neu=1')}>Neuer Vorgang</Button>} />

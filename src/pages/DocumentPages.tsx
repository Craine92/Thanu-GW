import {
  CheckCircle2, Copy, CreditCard, Ellipsis, Eye, FileDown, FilePenLine, FileSearch, MailCheck,
...
export function DocumentsListPage({ kind }: { kind: Kind }) {
  const { data, deleteQuote, duplicateQuote, convertQuoteToInvoice, duplicateInvoice, setInvoiceStatus, recordInvoicePayment, removeOrCancelInvoice } = useApp();
  ...
  async function confirmAction() {
    ...
    } catch { /* Der AppContext zeigt die fachliche Fehlermeldung als Toast an. */ }
  }
  async function submitPayment() {
    if (!paymentInvoice || payment <= 0) return;
    try {
      await recordInvoicePayment(paymentInvoice.id, payment);
      setPaymentInvoice(null); setPayment(0);
    } catch { /* Der AppContext zeigt die fachliche Fehlermeldung als Toast an. */ }
  }
  ...
  const quoteActions: RowAction<Quote>[] = [
    ...
    { label: 'PDF Vorschau', icon: FileSearch, onClick: (row) => previewPdf('quote', row) },
  ...
  const invoiceActions: RowAction<Invoice>[] = [
    ...
    { label: 'PDF Vorschau', icon: FileSearch, onClick: (row) => previewPdf('invoice', row) },
    { label: 'Duplizieren', icon: Copy, onClick: duplicate },
    { label: 'Als versendet markieren', icon: Send, hidden: (row) => row.status !== 'draft', onClick: (row) => setInvoiceStatus(row.id, 'sent') },
    { label: 'Zahlung erfassen', icon: CreditCard, hidden: (row) => ['paid', 'cancelled'].includes(row.status), onClick: (row) => { setPaymentInvoice(row); setPayment(0); } },
  ...
  return <div className="page"><PageHeader eyebrow="Verkauf" title={kind === 'quote' ? 'Kostenvoranschläge' : 'Rechnungen'} description={kind === 'quote' ? 'Offerten erstellen, nachverfolgen und direkt in Rechnungen überführen.' : 'Rechnungen erstellen, Zahlungen erfassen und Fälligkeiten verfolgen.'} actions={<Button icon={Plus} onClick={() => navigate(`/${kind === 'quote' ? 'kostenvoranschlaege' : 'rechnungen'}/neu`)}>{kind === 'quote' ? 'Neuer Kostenvoranschlag' : 'Neue Rechnung'}</Button>} />
    ...
    <Modal open={Boolean(paymentInvoice)} onClose={() => { setPaymentInvoice(null); setPayment(0); }} title="Zahlung erfassen" size="small" footer={<><Button variant="secondary" onClick={() => { setPaymentInvoice(null); setPayment(0); }}>Abbrechen</Button><Button onClick={submitPayment} disabled={payment <= 0}>Zahlung verbuchen</Button></>}>
      <label className="field"><span>Zahlungsbetrag ({paymentInvoice?.currency})</span><input type="number" min="0.01" max={paymentInvoice ? Math.max(0, paymentInvoice.total - paymentInvoice.paidAmount) : undefined} step="0.01" value={payment || ''} onChange={(event) => setPayment(Number(event.target.value))}/>{paymentInvoice && <small>Offen: {formatCurrency(Math.max(0, paymentInvoice.total - paymentInvoice.paidAmount), paymentInvoice.currency)}</small>}</label>
    </Modal>
  </div>;
}
...
export function DocumentDetailPage({ kind }: { kind: Kind }) {
  ...
  async function duplicate() {
    try {
      const created = kind === 'quote' ? await duplicateQuote(id) : await duplicateInvoice(id);
      navigate(`/${kind === 'quote' ? 'kostenvoranschlaege' : 'rechnungen'}/${created.id}`);
    } catch { /* Der AppContext zeigt die fachliche Fehlermeldung als Toast an. */ }
  }
  async function confirmAction() {
